...
class FLExlexicon(XMLCorpusReader):
    """Corpus reader for FLEx's lift files.
    ...
    def __init__(self, root, fileid):
        ...
        self._fileid = fileid
        self.elt = self.xml()
        self.entries = self.elt.findall('entry')
    ...
    def getGuid(self, entry):
        """Returns the guid of an entry."""
        return entry.get('guid')
        
    def getFields(self, entry):
        """Returns the list of the fields in an entry."""
        pass
        
    def getValue(self, entry, field):
        """Returns the value of field in entry."""
        pass
...
class FLExtext(XMLCorpusReader):
